...
reviews = []
for idx, url in enumerate(list_review_url[651:701]):
    driver.get(url)
    time.sleep(0.5)
    review = ''
    for i in range(1, 31):
...
df = pd.DataFrame({'title':movie_titles[651:701], 'reviews':reviews})
today = datetime.datetime.now().strftime('%Y%m%d')
df.to_csv('./crawling_data/reviews_700.csv'.format(today), index=False)